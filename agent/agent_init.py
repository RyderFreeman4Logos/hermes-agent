...
def init_agent(
    ...
    # Keep caller-owned request fields separate from route-derived fields.
    # Provider switches may replace the latter, but must never discard or
    # mutate explicit caller headers/body/timeout values.
    ...
    agent.request_overrides.update(
        _copy.deepcopy(agent._caller_request_overrides)
    )
