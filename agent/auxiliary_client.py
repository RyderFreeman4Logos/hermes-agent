...
def aux_host_candidate_deadline(seconds, *, total_deadline=None, idle_timeout=None, fence=None):
    """Bind auxiliary work to one host fence. Pin adapter stores the marker."""
...
def _create_with_progress(
    ...
    *,
    ...
    if not _aux_progress_active() and not force_stream:
        if not _aux_candidate_deadline_active() or _client_streams_internally(client):
            if not _client_streams_internally(client):
                from agent import relay_llm
        ...
        try:
            wait = float(wait) if wait is not None else 1.0
        except (TypeError, ValueError):
            wait = 1.0
        if wait <= 0:
            wait = 1.0
        ...
        response = _create_bounded(_create, wait)
        _notify_aux_provider_response()
        return response
