...
class _FallbackDestination(NamedTuple):
    ...
    model: Optional[str]
    credential_discriminator: str = ""


class _FallbackRoute(str):
    """JSON-safe selected fallback label with a non-secret route snapshot."""
    __slots__ = ("destination", "fingerprint")

    def __new__(cls, label: str, destination: _FallbackDestination, fingerprint: tuple) -> "_FallbackRoute":
...
class _SelectedRouteInfo(dict):
    """Public route metadata plus one process-local selected fallback route."""
...
def _set_selected_fallback_route(route_info: Any, route: Optional[_FallbackRoute]) -> None:
    if isinstance(route_info, _SelectedRouteInfo):
        route_info.fallback_route = route


def _fallback_route_fingerprint(entry: Dict[str, Any]) -> tuple:
    """Compare mutable route config without retaining credentials."""
    return (
        ...
        "inline" if entry.get("api_key") is not None else "default",
    )


def _fallback_route_for_client(task: Optional[str], fb_label: str, fb_client: Any, fb_model: Optional[str]) -> Optional[_FallbackRoute]:
    entry = _fallback_chain_entry(task, fb_label)
    if entry is None:
        return None
    fingerprint = getattr(fb_client, "_hermes_fallback_snapshot", None)
    if not isinstance(fingerprint, tuple):
        fingerprint = _fallback_route_fingerprint(entry)
    destination = _fallback_destination_from_entry(entry, fb_client, fb_model)._replace(
        credential_discriminator=fingerprint[-1],
    )
    return _FallbackRoute(fb_label, destination, fingerprint)


def _fallback_route_index(label: str) -> int:
    match = re.match(r"fallback_chain\[(\d+)\]", label or "")
    return int(match.group(1)) if match else 0


def _selected_route_is_current(route: _FallbackRoute, entry: Optional[Dict[str, Any]]) -> bool:
    return entry is not None and route.fingerprint == _fallback_route_fingerprint(entry)
...
def _call_fallback_candidate_sync(fb_client: Any, fb_model: Optional[str], fb_label: str, **kwargs: Any) -> Optional[Any]:
    """Apply a selected fallback's own limit without double-acquiring it."""
    ...
    if semaphore is not None and not owns_semaphore:
        semaphore.acquire()
    try:
        return _call_fallback_candidate_sync_impl(fb_client, fb_model, fb_label, **kwargs)
    finally:
        if semaphore is not None and not owns_semaphore:
            semaphore.release()
...
async def _call_fallback_candidate_async(fb_client: Any, fb_model: Optional[str], fb_label: str, **kwargs: Any) -> Optional[Any]:
    """Apply a selected fallback's own limit without double-acquiring it."""
    ...
    try:
        return await _call_fallback_candidate_async_impl(fb_client, fb_model, fb_label, **kwargs)
    finally:
        if semaphore is not None and not owns_semaphore:
            semaphore.release()
...
def _try_configured_fallback_chain(
    ...
    attempted_indices = attempted_indices or set()
    for i, entry in enumerate(chain):
        if i < start_index or i in attempted_indices or not isinstance(entry, dict):
            continue
...
def _normalize_fallback_base_url(value: Any) -> tuple:
    """Return a credential-free endpoint identity for route comparisons."""
    ...
    return (
        "endpoint", parsed.scheme.lower(), hostname, parsed.port,
        hashlib.blake2b(
            (parsed.path.rstrip("/") + "?" + parsed.query + "#" + parsed.fragment).encode(),
            digest_size=32,
        ).digest(),
    )
...
def _validate_fallback_base_url(value: Any) -> str:
    """Reject malformed fallback URLs before resolving a credentialed client."""
    if value is None:
        return ""
    base_url = str(value)
...
def _resolve_fallback_entry(entry: Dict[str, Any]) -> Tuple[Optional[Any], Optional[str]]:
    ...
    if not provider or not model:
        return None, None
    raw_base_url = selected.get("base_url")
    base_url = _validate_fallback_base_url(raw_base_url) if raw_base_url else None
    api_key = _fallback_entry_api_key(selected)
    api_mode = str(selected.get("api_mode") or selected.get("transport") or "").strip() or None
    client, resolved_model = resolve_provider_client(provider, model=model, explicit_base_url=base_url, explicit_api_key=api_key, api_mode=api_mode)
    if client is not None:
        try:
            client._hermes_fallback_destination = _fallback_destination_from_entry(selected, client, resolved_model)
            client._hermes_fallback_snapshot = _fallback_route_fingerprint(selected)
        except Exception:
            pass
...
def _get_task_max_concurrency(task: Optional[str], fallback_label: Optional[str] = None) -> Optional[int]:
...
def _acquire_sync_aux_semaphore(task: Optional[str], fallback_label: Optional[str] = None) -> Optional[threading.BoundedSemaphore]:
...
def _acquire_async_aux_semaphore(task: Optional[str], fallback_label: Optional[str] = None):
...
def call_llm(
    ...
    *,
    ...
    fallback_label = str(route_info.get("fallback_label") or "") if isinstance(route_info, dict) else ""
...
def _call_llm_impl(
    ...
    *,
    ...
    if isinstance(selected_route, _FallbackRoute):
        ...
        selected_error = None
        if _selected_route_is_current(selected_route, entry):
            route_client, route_model = _resolve_fallback_entry(entry)
            if route_client is not None:
                _record_route_info(
                    route_info, selected_route.destination.provider, route_model,
        ...
        next_client, next_model, next_label = _try_configured_fallback_chain(
            ...
            start_index=_fallback_route_index(str(selected_route)) + 1,
        )
        if next_client is not None:
            next_route = _fallback_route_for_client(task, next_label, next_client, next_model)
            _set_selected_fallback_route(route_info, next_route)
            _record_route_info(
            ...
            return _call_fallback_candidate_sync(
                next_client, next_model, next_label, task=task, messages=messages,
    ...
    try:
        ...
        if should_fallback and (is_auto or is_capacity_error):
            ...
            if fb_client is not None:
                ...
                _set_selected_fallback_route(
                    route_info, _fallback_route_for_client(task, fb_label, fb_client, fb_model),
                )
                try:
                ...
                _set_selected_fallback_route(route_info, None)
                next_client, next_model, next_label = _try_configured_fallback_chain(
                    task, _fallback_provider_from_label(fb_label),
                    failed_model=fb_model,
                    start_index=_fallback_route_index(fb_label) + 1,
                )
                if next_client is not None:
                    _set_selected_fallback_route(
                        route_info, _fallback_route_for_client(task, next_label, next_client, next_model),
                    )
                    _record_route_info(
                        route_info, _fallback_provider_from_label(next_label), next_model,
                        fallback_label=next_label,
    ...
    *,
    ...
    if isinstance(selected_route, _FallbackRoute):
        ...
        if _selected_route_is_current(selected_route, entry):
            route_client, route_model = _resolve_fallback_entry(entry)
            if route_client is not None:
                route_client, route_model = _to_async_client(
                    route_client, route_model or "", is_vision=(task == "vision"),
                )
                _record_route_info(
                    route_info, selected_route.destination.provider, route_model,
                    fallback_label=str(selected_route),
    ...
    try:
        ...
        if should_fallback and (is_auto or is_capacity_error):
            ...
            if fb_client is not None:
                ...
                _set_selected_fallback_route(
                    route_info, _fallback_route_for_client(task, fb_label, async_fb, async_fb_model or fb_model),
                )
                try:
                    fb_resp = await _call_fallback_candidate_async(
                        ...
                        max_tokens=max_tokens, tools=tools, effective_timeout=effective_timeout,
                        effective_extra_body=effective_extra_body, reasoning_config=reasoning_config)
                except AuxiliaryExplicitCancellation:
                    raise
                except Exception:
                    fb_resp = None
                ...
                next_client, next_model, next_label = _try_configured_fallback_chain(
                    task, _fallback_provider_from_label(fb_label),
                    failed_model=async_fb_model or fb_model,
                    start_index=_fallback_route_index(fb_label) + 1,
                )
                if next_client is not None:
                    next_client, next_model = _to_async_client(next_client, next_model or "", is_vision=(task == "vision"))
                    _set_selected_fallback_route(route_info, _fallback_route_for_client(task, next_label, next_client, next_model))
                    _record_route_info(route_info, _fallback_provider_from_label(next_label), next_model, fallback_label=next_label)
                    fb_resp = await _call_fallback_candidate_async(
                        next_client, next_model, next_label, task=task, messages=messages,
                        temperature=temperature, max_tokens=max_tokens, tools=tools,
                        effective_timeout=effective_timeout, effective_extra_body=effective_extra_body,
                        reasoning_config=reasoning_config)
                    if fb_resp is not None:
                        return fb_resp
