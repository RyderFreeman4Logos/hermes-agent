...
class _ACPChatCompletions:
    ...
    @staticmethod
    def heartbeat_warm_prepare(
        kwargs: dict[str, Any], _mode: str
    ) -> dict[str, Any]:
        # Tool choice is rendered into ACP's prompt text, so preserve it byte
        # for byte. Warm mode blocks client I/O and cancels on first update.
        kwargs["stream"] = True
        return kwargs
        ...
        def _request(method: str, params: dict[str, Any], *, text_parts: list[str] | None = None, reasoning_parts: list[str] | None = None) -> Any:
            ...
            while time.monotonic() < deadline:
                ...
                if self._handle_server_message(
                    ...
                    deny_client_io=cancel_after_first_update,
                ):
                    if (
    ...
    def _handle_server_message(
        ...
        *,
        ...
        if method == "session/request_permission":
            # ACP requires cancelled (not a JSON-RPC error) for pending
            # permission requests when the surrounding turn is cancelled.
            response = _permission_denied(message_id)
        elif deny_client_io:
            response = _jsonrpc_error(
                message_id,
                -32603,
                "Client I/O is disabled during heartbeat warm replay.",
            )
