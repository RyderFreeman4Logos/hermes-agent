...
A thinking/stream loop can emit 100k+ chars over several minutes, then leave
that orphan as the last assistant row when the user interrupts. Bound the
live payload in one place so every stream writer and the interrupt persist
path share the same check.
"""
...
def persist_interrupted_stream_partial(
    ...
    *,
    ...
    A runaway-repetition interrupt is already a hidden placeholder; do not
    replace that row with the streamed partial.
    """
    if bound is None:
    ...
    if exceeded or visible_size > bound:
        text = stream_payload_error_text(int(size) or visible_size, bound)
        try:
