...
def get_model_switch_after_compression(agent: Any) -> Optional[ModelSwitchResult]:
    """Return the currently scheduled post-compression switch, if any."""
...
def schedule_model_switch_after_compression(
    ...
    *,
    ...
    with model_switch_transaction_lock(agent):
        ...
        if result.reasoning_config is not None:
            config[_AFTER_COMPRESSION_CONFIG_KEY]["reasoning_config"] = copy.deepcopy(
...
def restore_model_switch_after_compression(agent: Any) -> Optional[ModelSwitchResult]:
    """Rehydrate a persisted post-compression switch after a restart."""
    ...
    if isinstance(descriptor.get("reasoning_config"), dict):
        result.reasoning_config = copy.deepcopy(descriptor["reasoning_config"])
    ...
    with model_switch_transaction_lock(agent):
        ...
        try:
            ...
            _persist_session_model_config(
                agent,
                _applied_model_config(agent, result),
                model=result.new_model,
                result=result,
            ...
            for name in (
                "_deferred_model_switch_context_length",
                "_deferred_model_switch_reasoning_config",
            ):
                try:
                    delattr(agent, name)
                except AttributeError:
...
def switch_model(
    ...
    if not new_model and reasoning and not explicit_provider:
        raw_input = current_model
        new_model = current_model.strip()
    ...
    if validate_live:
        ...
        # Deferred scheduling resolves static route metadata without probing a
        # live provider. The compression boundary owns the eventual swap.
        validation = {
            "accepted": True,
            "persist": True,
            "recognized": True,
