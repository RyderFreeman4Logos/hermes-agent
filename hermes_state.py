...
def _uses_delete_journal_on_disk(db_path: Path) -> bool:
    ...
    # Use the pin's tracked pre-open reader rather than opening state.db
    # directly: on POSIX, closing any fd for the same inode can release this
    # process's SQLite locks while another connection is live.
...
def repair_state_db_schema(db_path: Path, *, backup: bool = True) -> Dict[str, Any]:
    ...
    with _session_db_advisory_write_lock(
        ...
        return _repair_state_db_schema_coordinated(db_path, backup=backup)


def _repair_state_db_schema_coordinated(
...
def _repair_state_db_schema_locked(
    ...
    Caller must hold both the advisory writer lock and the repair lock.
    """
