...
def _fallback_test_config(limit):
    return {
        "fallback_chain": [{
...
def _run_sync_fallback_limit(limit, n_callers):
    ...
    with _patch_fallback_probe(_fallback_test_config(limit), primary, fallback, async_mode=False):
        threads = [threading.Thread(target=worker) for _ in range(n_callers)]
        for thread in threads:
            thread.start()
        assert primary_ready.wait(2) and candidate_ready.wait(2)
...
async def _run_async_fallback_limit(limit, n_callers):
    target = 1 if limit == 1 else n_callers
    ...
    with _patch_fallback_probe(_fallback_test_config(limit), primary, fallback, async_mode=True):
...
def test_sync_fallback_route_setup_failure_releases_candidate_permit():
    ...
    original_route = auxiliary_client._fallback_route
    route_calls = 0
    ...
    with ExitStack() as stack:
        ...
        stack.enter_context(patch("agent.auxiliary_client._fallback_route", side_effect=build_route))
        with pytest.raises(RuntimeError, match="route setup failed"):
            call_llm(task="compression", messages=[{"role": "user", "content": "first"}])
...
async def test_selected_async_cancellation_releases_permit_without_replan():
    ...
    assert "_fallback_route" in route_info
