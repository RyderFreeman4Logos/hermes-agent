...
def _usage(*, cache_read: int, prompt: int) -> CanonicalUsage:
...
def _dump(dump, *, cache_read: int, prompt: int, telemetry: str = "reported") -> None:
...
def test_near_zero_sub_percent_read_dumps_last_two_fingerprints(monkeypatch, tmp_path):
    ...
    _dump(dump, cache_read=512, prompt=60_246)
...
def test_high_hit_does_not_dump(monkeypatch, tmp_path):
    ...
    _dump(dump, cache_read=9_000, prompt=10_000)
...
def test_unavailable_telemetry_does_not_dump(monkeypatch, tmp_path):
    ...
    _dump(dump, cache_read=0, prompt=10_000, telemetry="unavailable")
