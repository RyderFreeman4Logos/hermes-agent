...
def test_full_in_place_compression_atomically_clears_durable_prune_runway(
    ...
    stored_config = json.loads(db.get_session(session_id)["model_config"])
    assert stored_config == agent._session_init_model_config
    assert "_proactive_prune_rearm_tokens" not in stored_config
