...
def test_selected_fallback_route_reuses_exact_transport_identity_for_siblings(caplog):
    ...
    for _provider, _model, kwargs in resolve_calls:
        assert kwargs["explicit_base_url"] == "https://fallback.invalid/anthropic"
        assert kwargs["explicit_api_key"] == "SENTINEL_PROFILE_KEY"
...
@pytest.mark.parametrize("failure_mode", ["raise", "rejected"])
...
@pytest.mark.asyncio
@pytest.mark.parametrize("async_mode", [False, True])
@pytest.mark.parametrize("selected_none", [False, True])
async def test_selected_route_none_continues_chain_without_rewalking_healthy_route(
    async_mode, selected_none, caplog,
):
    primary_calls, first_calls, second_calls, candidate_calls = [], [], [], []
    make = AsyncMock if async_mode else MagicMock
    ...
    first = _client(
        ...
        api_mode="anthropic_messages", credential_scope="FIRST_SELECTED_SECRET",
    )
    second = _client(
        make(side_effect=second_create), endpoint="https://second.invalid/anthropic",
        api_mode="anthropic_messages", credential_scope="SECOND_HEALTHY_SECRET",
    )
    config = _two_fallback_config()
    config["fallback_chain"][0]["api_key"] = "FIRST_SELECTED_SECRET"
    config["fallback_chain"][1]["api_key"] = "SECOND_HEALTHY_SECRET"
    route_info = _SelectedRouteInfo()
    ...
    with ExitStack() as stack:
        ...
        first_response = await first_response if async_mode else first_response
        assert first_response.choices[0].message.content == "FIRST"
        primary_calls_after_first = len(primary_calls)

        first_label = "fallback_chain[0](first-provider)"
        second_label = "fallback_chain[1](second-provider)"

        def selected_sync(client, model, label, **kwargs):
            candidate_calls.append((client, model, label))
            if selected_none and label == first_label:
                return None
            return _call_fallback_candidate_sync(client, model, label, **kwargs)

        async def selected_async(client, model, label, **kwargs):
            candidate_calls.append((client, model, label))
            if selected_none and label == first_label:
                return None
            return await _call_fallback_candidate_async(client, model, label, **kwargs)

        if async_mode:
            stack.enter_context(patch(
                "agent.auxiliary_client._call_fallback_candidate_async",
                new=AsyncMock(side_effect=selected_async),
            ))
        else:
            stack.enter_context(patch(
                "agent.auxiliary_client._call_fallback_candidate_sync",
                side_effect=selected_sync,
            ))
        chain_spy = stack.enter_context(patch(
            "agent.auxiliary_client._try_configured_fallback_chain",
            wraps=_try_configured_fallback_chain,
        ))

        response = request(
            task="compression", messages=[{"role": "user", "content": "second"}],
            route_info=route_info,
        )
        response = await response if async_mode else response

    expected_label = second_label if selected_none else first_label
    expected_provider = "second-provider" if selected_none else "first-provider"
    expected_model = "second-model" if selected_none else "first-model"
    expected_body = "SECOND" if selected_none else "FIRST"
    assert response is not None
    assert response.choices[0].message.content == expected_body
    assert response.choices[0].message.content != "None"
    assert [label for _, _, label in candidate_calls] == (
        [first_label, second_label] if selected_none else [first_label]
    )
    assert [label for _, _, label in candidate_calls].count(first_label) == 1
    if selected_none:
        assert chain_spy.call_count == 1
        assert chain_spy.call_args.args == ("compression", "first-provider")
        assert chain_spy.call_args.kwargs["failed_model"] == "first-model"
        assert chain_spy.call_args.kwargs["start_index"] == 1
        assert candidate_calls[1][0] is second
    else:
        chain_spy.assert_not_called()
    assert route_info["provider"] == expected_provider
    assert route_info["model"] == expected_model
    assert route_info["fallback_label"] == expected_label
    assert route_info.fallback_route.destination.provider == expected_provider
    assert route_info.fallback_route.destination.model == expected_model
    assert len(primary_calls) == primary_calls_after_first
    assert len(first_calls) == (1 if selected_none else 2)
    assert len(second_calls) == (1 if selected_none else 0)
    visible = " ".join((repr(route_info), json.dumps(route_info), caplog.text))
    assert "FIRST_SELECTED_SECRET" not in visible
    assert "SECOND_HEALTHY_SECRET" not in visible
    assert "api_key" not in visible
    assert "client" not in repr(route_info)
    pickle.dumps(route_info)


@pytest.mark.parametrize("first_mode", ["raise"])
...
def test_fallback_userinfo_is_identity_safe():
    from agent.auxiliary_client import _normalize_fallback_base_url

    sentinel = "USERINFO_SENTINEL"
    normalized = _normalize_fallback_base_url(
        f"https://{sentinel}:PASSWORD_SENTINEL@fallback.invalid/anthropic"
    )

    assert "fallback.invalid" in repr(normalized)
    assert sentinel not in repr(normalized)
    assert "PASSWORD_SENTINEL" not in repr(normalized)
...
async def test_selected_route_mutation_replans_without_mixing_client_identity(async_mode):
    ...
    assert resolve_calls == [first.base_url, second.base_url]
...
async def test_selected_route_inline_key_mutation_during_resolution_replans_without_using_new_key(
    ...
    with ExitStack() as stack:
        ...
        primary_calls_after_first = len(primary_calls)
        selected = route_info.fallback_route.destination
    ...
    assert len(second_calls) == 1
    assert resolve_keys == [key_a, None]
...
def test_lean_chunk_digests_default_to_two_workers_when_limit_is_absent():
    """An omitted compression limit is the documented two-worker default."""
    active = 0
    max_active = 0
    lock = threading.Lock()
    ...
    _run_retry_digest(fake_call_llm, {}, count=4)
