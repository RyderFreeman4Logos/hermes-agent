...
class TestMicroCompaction:
    ...
    def test_cadence_noop_does_not_scan_durable_transcript(self):
        cc = _compressor()
        cc._micro_compact_every_n_turns = 3
        session_db = MagicMock()
        cc._session_db = session_db
        cc._session_id = "cadence-noop"

        messages = _conversation(exchanges=8)
        assert cc._micro_compact(list(messages)) == messages

        session_db.get_compaction_baseline.assert_not_called()
