...
def test_reasoning_only_deferred_resolution_keeps_current_model_and_effort():
    result = switch_model(
        raw_input="",
        current_provider="openrouter",
        current_model="old-model",
    ...
    assert result.success, result.error_message
    assert result.new_model == "old-model"
    assert result.reasoning_config == {"enabled": True, "effort": "low"}


def test_cli_after_compression_command_schedules_instead_of_live_switch(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda: SimpleNamespace(
            ...
            with_overrides=lambda **_k: SimpleNamespace(
    ...
    cli_mod.HermesCLI._handle_model_switch(
        StubCLI(), "/model new-model --after-compression --reasoning low"
    )

    assert resolved["is_after_compression"] is True
    assert resolved["reasoning"] == "low"
    assert resolved["validate_live"] is False
    assert len(scheduled) == 1
    assert scheduled[0][1].new_model == "new-model"
