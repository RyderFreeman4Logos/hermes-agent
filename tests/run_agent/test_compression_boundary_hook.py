...
class TestCompressionBoundaryHook:
    ...
    def test_publication_failure_rolls_back_switch_and_keeps_pending(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert scheduled_route[3] is None
            ...
            # Publication failure aborts before the committed boundary, so the
            # deferred switch must not run and remains pending for retry.
            assert agent.switch_model.call_count == 0
            assert get_model_switch_after_compression(agent) is pending
    ...
    def test_billing_mode_none_explicitly_clears_rollback_route(self):
        """An explicit NULL billing mode must restore a NULL preimage."""
        from hermes_state import SessionDB

        with tempfile.TemporaryDirectory() as tmpdir:
            db = SessionDB(db_path=Path(tmpdir) / "test.db")
            session_id = "billing-mode-rollback"
            db.create_session(session_id=session_id, source="cli", model="old-model")
            db.update_session_billing_route(
                session_id,
                provider="new-provider",
                base_url="https://new.example/v1",
                billing_mode="new-api-mode",
            )
            db.update_session_billing_route(
                session_id,
            ...
            assert restored["billing_provider"] == "old-provider"
            assert restored["billing_base_url"] == "https://old.example/v1"
            assert restored["billing_mode"] is None
