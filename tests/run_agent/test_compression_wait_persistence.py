...
def _observe_busy(db: SessionDB, monkeypatch) -> threading.Event:
    ...
    monkeypatch.setattr(db, "append_message", _append)
    return busy
...
def test_successor_publication_race_adoption_is_idempotent(
    ...
    monkeypatch.setattr(db, "append_message", _publish_between_checks)
    agent = _agent(db)
...
def test_active_lease_without_successor_times_out_fatally() -> None:
    session_db = SimpleNamespace(
        append_message=MagicMock(side_effect=CompressionSessionBusyError("busy")),
    ...
    assert elapsed >= 0.08
    assert 1 < session_db.append_message.call_count < 10
...
def test_real_storage_error_keeps_exact_fatal_safeguard() -> None:
    session_db = SimpleNamespace()
    session_db.append_message = MagicMock(
    ...
    session_db.append_message.assert_called_once()
