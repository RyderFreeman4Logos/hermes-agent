...
def test_runtime_projection_is_pure_for_canonical_target():
    from agent.agent_runtime_helpers import _project_request_overrides_for_runtime
    ...
    projected = _project_request_overrides_for_runtime(
        agent,
        provider="CUSTOM:ZAI-CODING-PLAN",
        base_url=f"{_ZAI_BASE_URL}/",
        model="GLM-5.2",
        service_tier=None,
    )
    ...
    assert projected == {}
    assert projected is not agent.request_overrides


def test_runtime_override_projection_recomputes_fast_mode_for_target_model():
    from agent.agent_runtime_helpers import _project_request_overrides_for_runtime
    ...
    assert projected == {"speed": "fast"}
    assert agent.request_overrides == _STALE_OVERRIDES


def test_runtime_override_projection_rejects_payload_without_log_egress(
    caplog, monkeypatch
):
    ...
    calls = []
    payload_marker = "PAYLOAD-SENTINEL-41"
    exception_marker = "EXCEPTION-SENTINEL-41"

    class HostileCopy:
        def __deepcopy__(self, _memo):
            calls.append("copy")
            raise ValueError(exception_marker)

        def __repr__(self):
            return payload_marker

    safe = {"safe": {"owned": True}}
    agent = _make_zai_agent()
    agent.request_overrides = safe
    agent._caller_request_overrides = {"hostile": HostileCopy()}
    monkeypatch.setattr(agent, "provider", "anthropic")
    monkeypatch.setattr(agent, "model", "claude-opus-4-6")
    monkeypatch.setattr(agent, "base_url", "https://api.anthropic.com")

    with caplog.at_level("DEBUG", logger="agent.agent_runtime_helpers"):
...
def test_pr41_r06_public_switch_rejection_aborts_complete_runtime():
    calls = []
    ...
    target_url = "https://rejected.example/v1"
    providers = [
        {
    ...
    with (
        ...
        patch("hermes_cli.timeouts.get_provider_request_timeout", return_value=None),
        pytest.raises(TypeError, match="request override projection rejected"),
    ):
        agent.switch_model(
            new_model="rejected-model",
            new_provider="custom:rejected",
    ...
    assert agent.base_url == old["base_url"]
    assert agent.api_mode == old["api_mode"]
    assert agent.client is old["client"]
    assert agent._credential_pool is old["pool"]
    assert agent.request_overrides is old["request_overrides"]


def test_pr41_r07_rejected_fallback_advances_to_valid_candidate():
    calls = []
    ...
    bad_url = "https://bad.example/v1"
    good_url = "https://good.example/v1"
    bad_client = _fallback_client(bad_url, "bad-key")
    good_client = _fallback_client(good_url, "good-key")
    agent = _make_fallback_agent(
        [
            {"provider": "custom:bad", "model": "bad-model", "base_url": bad_url},
            {"provider": "custom:good", "model": "good-model", "base_url": good_url},
        ]
    )
    patches = _fallback_patches(
        clients=[(bad_client, "bad-model"), (good_client, "good-model")],
        custom_providers=[
            {
                "provider_key": "bad",
                "base_url": bad_url,
    ...
    assert calls == []
    assert agent.model == "good-model"
    assert agent.provider == "custom:good"
    assert agent.request_overrides == {"extra_body": {"candidate": "good"}}
...
def test_pr41_r08_restore_projection_rejection_leaves_fallback_untouched():
    calls = []
    ...
    with (
        ...
        patch(
            "hermes_cli.config.get_compatible_custom_providers",
            return_value=[primary_config],
        ),
    ):
        assert agent._restore_primary_runtime() is False

    assert calls == []
    assert agent.model == active[0]
    assert agent.provider == active[1]
    assert agent.base_url == active[2]
    assert agent.client is active[3]
    assert agent.request_overrides is active[4]
    assert agent._fallback_activated is active[5]
    assert agent._fallback_index == active[6]


def test_pr41_r09_recovery_rejection_precedes_client_retirement():
    from agent.agent_runtime_helpers import try_recover_primary_transport
    ...
    agent._vprint = MagicMock()
    safe_client = agent.client
    safe_map = agent.request_overrides
    primary_config = {
        "provider_key": "zai-coding-plan",
        "base_url": _ZAI_BASE_URL,
        "model": "glm-5.2",
    ...
    with (
        ...
        assert try_recover_primary_transport(
            agent, error, retry_count=3, max_retries=3
        ) is False
