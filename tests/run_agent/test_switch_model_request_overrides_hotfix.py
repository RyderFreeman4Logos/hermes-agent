...
def test_fallback_snapshot_rejects_hostile_enum_before_deepcopy():
    ...
    agent = types.SimpleNamespace(
        _client_kwargs={"hostile": HostileEnum.VALUE},
        request_overrides={},
        reasoning_config={},
        _transport_cache={},
    )
...
def test_fallback_snapshot_rejects_spoofed_and_subclassed_httpx_handles():
    import httpx
    ...
    class ClientSubclass(httpx.Client):
        pass

    values = [Client(), ClientSubclass()]
    try:
        for value in values:
            for field, graph in (
                ("_client_kwargs", {"arbitrary": value}),
                ("request_overrides", {"arbitrary": value}),
                ("reasoning_config", {"arbitrary": value}),
            ):
                agent = types.SimpleNamespace(
                    _client_kwargs={},
                    request_overrides={},
...
def test_custom_provider_extra_body_is_deep_copied_at_both_boundaries():
    ...
    _compose_request_overrides(agent, {}, custom_providers=providers)
    agent.request_overrides["extra_body"]["nested"]["items"].append("active")
    assert configured["nested"]["items"] == ["configured"]


def test_pr41_r01_export_replacement_cannot_change_snapshot_trust(monkeypatch):
    import httpx
    ...
    try:
        agent = types.SimpleNamespace(
            model="primary",
            _client_kwargs={"http_client": client},
            request_overrides={"safe": {"nested": True}},
            reasoning_config={},
            _transport_cache={},
        )
        snapshot = _snapshot_fallback_candidate_runtime(agent)
        agent.model = "candidate"
        agent.request_overrides = {"candidate": True}

        class ForgedClient:
            pass

        monkeypatch.setattr(httpx, "Client", ForgedClient)
        _restore_fallback_candidate_runtime(agent, snapshot)

        assert agent.model == "primary"
        assert agent._client_kwargs["http_client"] is client
        assert agent.request_overrides == {"safe": {"nested": True}}

        forged = types.SimpleNamespace(
            _client_kwargs={},
            request_overrides={"client": ForgedClient()},
            reasoning_config={},
            _transport_cache={},
        )
        with pytest.raises(TypeError, match="unsupported fallback snapshot value"):
            _snapshot_fallback_candidate_runtime(forged)
    finally:
        client.close()


def test_pr41_r02_module_replacement_cannot_change_snapshot_trust(monkeypatch):
    import sys
    ...
    try:
        ...
        snapshot = _snapshot_fallback_candidate_runtime(agent)
        agent.model = "candidate"
        replacement = ModuleType("httpx")
        replacement.Client = type("ForgedClient", (), {})
        monkeypatch.setitem(sys.modules, "httpx", replacement)
...
def test_pr41_r03_generated_botocore_client_retains_owned_identity():
...
def test_pr41_r04_late_restore_rejection_performs_zero_writes():
    from agent.chat_completion_helpers import (
        _restore_fallback_candidate_runtime,
        _snapshot_fallback_candidate_runtime,
    )
    ...
    agent = types.SimpleNamespace(
        model="primary",
        provider="primary-provider",
        _client_kwargs={"safe": {"nested": True}},
        request_overrides={"safe": {"nested": True}},
        reasoning_config={"safe": {"nested": True}},
        _transport_cache={"primary": object()},
    )
    snapshot = _snapshot_fallback_candidate_runtime(agent)
    snapshot["reasoning_config"]["late"] = Hostile()
    agent.model = "candidate"
    agent.provider = "candidate-provider"
    candidate_kwargs = {"candidate": True}
    candidate_overrides = {"candidate": True}
    candidate_reasoning = {"candidate": True}
    candidate_transport = {"candidate": object()}
    agent._client_kwargs = candidate_kwargs
    agent.request_overrides = candidate_overrides
    agent.reasoning_config = candidate_reasoning
    agent._transport_cache = candidate_transport

    with pytest.raises(TypeError, match="unsupported fallback snapshot value"):
        _restore_fallback_candidate_runtime(agent, snapshot)

    assert calls == []
    assert agent.model == "candidate"
    assert agent.provider == "candidate-provider"
    assert agent._client_kwargs is candidate_kwargs
    assert agent.request_overrides is candidate_overrides
    assert agent.reasoning_config is candidate_reasoning
    assert agent._transport_cache is candidate_transport


def test_pr41_r05_custom_body_rejection_has_zero_hooks_and_publications():
    from agent.agent_init import _compose_request_overrides
    ...
    class Hostile:
        def __deepcopy__(self, _memo):
            calls.append("copy")
            raise RuntimeError("payload must stay private")

    safe = {"safe": {"nested": True}}
    agent = types.SimpleNamespace(
        provider="custom:demo",
        model="demo-model",
        base_url="https://demo.example/v1",
        service_tier=None,
        _caller_request_overrides={"caller": True},
        request_overrides=safe,
    )
    providers = [
        {
            "provider_key": "demo",
            ...
            "model": agent.model,
            "extra_body": {"hostile": Hostile()},
        }
    ]

    with pytest.raises(TypeError, match="request override projection rejected"):
        _compose_request_overrides(agent, {}, custom_providers=providers)
...
def test_pr41_r10_cached_turn_rejection_preserves_service_tier_and_map():
    from gateway.run import _apply_cached_turn_runtime
    ...
    agent = types.SimpleNamespace(
        ...
        base_url="https://demo.example/v1",
        service_tier=None,
        reasoning_config={"old": True},
        _caller_request_overrides={},
        request_overrides=safe,
        _custom_providers=[
            {
                "provider_key": "demo",
    ...
    with pytest.raises(TypeError, match="request override projection rejected"):
        _apply_cached_turn_runtime(
            agent,
            reasoning_config={"new": True},
            service_tier="priority",
            derived_overrides={"service_tier": "priority"},
        )

    assert calls == []
    assert agent.service_tier is None
    assert agent.reasoning_config == {"old": True}
    assert agent.request_overrides is safe
