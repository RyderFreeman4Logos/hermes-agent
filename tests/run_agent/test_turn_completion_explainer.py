...
def test_heartbeat_matches_ordinary_effective_cache_prefix(heartbeat_event):
    ...
    assert heartbeat_request.get("tool_choice") == ordinary_request.get("tool_choice")
...
def test_heartbeat_identity_exception_preserves_optional_claim_contract(
    monkeypatch,
):
    from tools.runtime_heartbeat import (
        claim_warm_snapshot,
    ...
    _prime_heartbeat_snapshot(agent)
    monkeypatch.setattr(
        "tools.runtime_heartbeat._warm_snapshot_identity",
        MagicMock(side_effect=RuntimeError("identity unavailable")),
    )

    assert claim_warm_snapshot(agent) is None
    assert warm_snapshot_claim_reason(agent) == "identity_unavailable"


def test_heartbeat_identity_exception_safe_skips_without_dispatch(
    heartbeat_event, monkeypatch
):
    agent = _make_agent(max_iterations=10)
    _prime_heartbeat_snapshot(agent)
    ...
    assert result["silent_noop"] is True
    assert result["heartbeat_warm_status"] == "skipped"
    assert result["heartbeat_warm_reason"] == "identity_unavailable"
    agent.client.chat.completions.create.assert_not_called()
...
def test_deepinfra_style_fake_upstream_warm_is_one_token_and_side_effect_free(
    ...
    try:
        ...
        assert warm_body.get("tool_choice") == normal_body.get("tool_choice")
        assert agent._session_messages is history
...
def test_copilot_acp_warm_uses_cancel_capable_adapter_contract(heartbeat_event):
    ...
    assert 'Tool choice hint: "auto"' in prompt.call_args.args[0]
    assert 'Tool choice hint: "none"' not in prompt.call_args.args[0]
