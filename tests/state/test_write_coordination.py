...
@contextmanager
def _advisory_holder(process_ctx, lock_path: Path, *, hold_s: float = 5.0):
...
def _assert_call_waits_for_advisory_lock(process_ctx, lock_path, call):
    ...
    with _advisory_holder(process_ctx, lock_path) as (holder, release):
...
def test_automatic_fts_merge_waits_for_advisory_lock(db, process_ctx) -> None:
    ...
    _assert_call_waits_for_advisory_lock(
        process_ctx,
        session_db_write_lock_path(db.db_path),
...
def test_schema_open_waits_for_advisory_lock(db, process_ctx) -> None:
    reopened = _assert_call_waits_for_advisory_lock(
        process_ctx,
        session_db_write_lock_path(db.db_path),
        lambda: SessionDB(db_path=db.db_path),
    )
...
def test_schema_repair_waits_for_advisory_lock(db, process_ctx) -> None:
    report = _assert_call_waits_for_advisory_lock(
        process_ctx,
        session_db_write_lock_path(db.db_path),
        lambda: hermes_state.repair_state_db_schema(db.db_path, backup=False),
    )
