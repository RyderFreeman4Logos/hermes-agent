...
def test_prompt_submit_emits_first_cache_status_before_later_tool_work(monkeypatch):
    cache_info = {
        "cached_tokens": 0,
        "level": "error",
        "prompt_tokens": 2_000,
    ...
    class _Agent:
        def run_conversation(
            ...
            server._emit(
                "tool.start",
                "sid",
                {"args": {}, "name": "terminal", "tool_id": "tool-1"},
            )
            return {
                "cache_info": cache_info,
                "completed": True,
    ...
    assert complete_payload["cache_info"] == cache_info
