...
def _dispatch_batch(
    runner, capacity, *, session_key="owner", interrupt_fn=None, on_not_started=None,
):
...
def test_persistence_failure_releases_backlog_reservation(monkeypatch, dispatch_one):
    ...
    result = dispatch_one(
        lambda: runner_started.set(), 1, on_not_started=callbacks.append
    )
...
def test_public_handler_cleans_resources_after_schedule_rejection(monkeypatch):
    ...
    result = json.loads(
        AIAgent._dispatch_delegate_task(
            ...
            {
                "tasks": [
                    {"goal": "first goal task"},
                    {"goal": "second goal task"},
                ]
            },
        )
    )
...
def test_public_force_background_rejection_finalizes_and_detaches_child(monkeypatch):
    parent = MagicMock(
        _delegate_depth=0,
        session_id="force-background-session",
        _interrupt_requested=False,
        _active_children=[],
        _active_children_lock=None,
    )
    ...
    def build_child(**kwargs):
        child = MagicMock(
            model="m",
            session_id="force-background-child",
            _delegate_role="leaf",
        )
        children.append(child)
        parent._active_children.append(child)
    ...
    monkeypatch.setattr("tools.delegate_tool._build_child_agent", build_child)
    monkeypatch.setattr("tools.delegate_tool._load_config", lambda: {"force_background": True})
    monkeypatch.setattr(
        "tools.delegate_tool._resolve_delegation_credentials",
        lambda *args, **kwargs: {
    ...
    result = json.loads(
        AIAgent._dispatch_delegate_task(parent, {"goal": "must not run inline"})
    )
