    ...
    def test_all_named_profile_paths_recheck_inheritance_at_builder(
        ...
        cases = (
            ...
            (
                ...
                {
                    "tasks": [
                        {"goal": "first delegated task"},
                        {"goal": "second delegated task"},
                    ],
                    "model_profile": "other",
                },
                0,
...
class TestDelegateTaskModelProfile(unittest.TestCase):
    ...
    def test_per_task_profile_overrides_top_level(self, mock_creds, mock_cfg):
        ...
        with patch("run_agent.AIAgent") as MockAgent:
            ...
            delegate_task(
                tasks=[
                    {"goal": "complete delegated task A", "model_profile": "fast"},
                    {"goal": "complete delegated task B", "model_profile": "smart"},
                ],
                parent_agent=parent,
            )
    ...
    def test_batch_prevalidates_all_profiles_before_spawning(self, MockAgent, mock_cfg):
        ...
        result = delegate_task(
            tasks=[
                {"goal": "first delegated task", "model_profile": "valid"},
                {"goal": "second delegated task", "model_profile": "invalid"},
            ],
