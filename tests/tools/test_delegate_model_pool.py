...
class TestBuildChildAgentCredentialInheritance(unittest.TestCase):
    ...
    def test_all_named_profile_paths_recheck_inheritance_at_builder(
        ...
        cases = (
            ("sync", {"goal": "sync task goal", "model_profile": "other"}, 0),
            (
                "background",
                {"goal": "background task", "model_profile": "other", "background": True},
                0,
            ),
            (
                "batch",
                {
                    "tasks": [{"goal": "first task goal"}, {"goal": "second task goal"}],
                    "model_profile": "other",
                },
                0,
            ),
            (
                "orchestrator",
                {"goal": "nested task goal", "model_profile": "other", "role": "orchestrator"},
                1,
            ),
        )
...
class TestDelegateTaskModelProfile(unittest.TestCase):
    ...
    def test_per_task_profile_overrides_top_level(self, mock_creds, mock_cfg):
        ...
        with patch("run_agent.AIAgent") as MockAgent:
            ...
            delegate_task(
                tasks=[
                    {"goal": "task A detail", "model_profile": "fast"},
                    {"goal": "task B detail", "model_profile": "smart"},
                ],
                parent_agent=parent,
            )
    ...
    def test_batch_prevalidates_all_profiles_before_spawning(self, MockAgent, mock_cfg):
        ...
        result = delegate_task(
            tasks=[
                {"goal": "first task goal", "model_profile": "valid"},
                {"goal": "second task goal", "model_profile": "invalid"},
            ],
