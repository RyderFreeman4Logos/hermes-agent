...
class TestReaderLoopOrphanedPipe:
    ...
    def test_final_notify_waits_for_child_and_is_exactly_once(self, registry, monkeypatch):
        from tools.runtime_heartbeat import runtime_heartbeat

        cancel_heartbeat = MagicMock()
        monkeypatch.setattr(runtime_heartbeat, "cancel", cancel_heartbeat)
        proc = subprocess.Popen(
            [
                sys.executable,
                "-c",
        ...
        assert registry.completion_queue.empty()
        cancel_heartbeat.assert_not_called()
        ...
        registry._move_to_finished(s)
        assert registry.completion_queue.empty()
        cancel_heartbeat.assert_called_once_with(s.id)
