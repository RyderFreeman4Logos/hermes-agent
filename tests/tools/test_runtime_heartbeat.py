...
def test_host_process_cpu_includes_identity_checked_descendants(monkeypatch):
    ...
    children = [
        SimpleNamespace(
            pid=200,
            cpu_times=lambda: SimpleNamespace(user=4.0, system=0.5),
        ),
        SimpleNamespace(pid=300),
    ]
    monkeypatch.setattr(
        psutil,
        "Process",
        lambda pid: SimpleNamespace(
            ...
            children=lambda recursive: children if pid == 100 else [],
        ),
    )
...
def test_linux_local_process_cpu_keeps_heartbeat_alive(
    ...
    expected_argv = [sys.executable, "-c", worker_code, str(stop)]
    ...
    try:
        ...
        assert worker_start == session.host_start_time
        ...
        assert baseline["output_size"] == len(session.output_buffer) == 0
