...
def _reset_bridge_state(monkeypatch):
    ...
    monkeypatch.delenv("TERMINAL_AUTO_BACKGROUND_TIMEOUT", raising=False)
...
def test_bridge_hot_reloads_auto_background_timeout_on_mtime_change():
    """Editing the dedicated promoted-wait timeout updates env without restart."""
    _write_config("terminal:\n  auto_background_timeout: 3300\n")
    config = terminal_tool._get_env_config()
    assert config["auto_background_timeout"] == 3300
    assert os.environ.get("TERMINAL_AUTO_BACKGROUND_TIMEOUT") == "3300"

    # Ensure mtime/size changes (tiny sleep + different content size).
    time.sleep(0.01)
    _write_config("terminal:\n  auto_background_timeout: 900\n  # pad for size change\n")

    config2 = terminal_tool._get_env_config()
    assert config2["auto_background_timeout"] == 900
    assert os.environ.get("TERMINAL_AUTO_BACKGROUND_TIMEOUT") == "900"
