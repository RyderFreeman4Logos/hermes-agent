"""message.complete stamps loop end time + first-call cache_info."""
...
def turn_env(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(server, "_apply_pending_model_switch", lambda sid, session: None)
    ...
    monkeypatch.setattr(server, "_record_turn_marker", lambda *a, **k: "")
...
def test_public_conversation_wrapper_forwards_turn_origin(monkeypatch):
    from agent import conversation_loop

    captured = {}

    def _run(_agent, _message, **kwargs):
        captured.update(kwargs)
        return {"final_response": "reply", "messages": []}

    monkeypatch.setattr(conversation_loop, "_run_conversation_turn", _run)
    monkeypatch.setattr(
    ...
    result = conversation_loop.run_conversation(
        object(), "wake", turn_origin="tui_cache_warm"
    )

    assert result["final_response"] == "reply"
    assert captured["turn_origin"] == "tui_cache_warm"
