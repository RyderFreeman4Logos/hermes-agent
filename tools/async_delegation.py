...
def _try_register_queued(record: Dict[str, Any]) -> bool:
    ...
    with _records_lock:
        ...
        try:
            ...
            record["_registration_error"] = f"Failed to schedule async delegation: {exc}"
            _invoke_not_started(record.get("_on_not_started"), "error")
            record["_not_started_called"] = True
            return False
...
def dispatch_async_delegation(
    *,
    ...
    if not _try_register_queued(record):
        if not record.pop("_not_started_called", False):
            _invoke_not_started(on_not_started, "error")
        return {
            "status": "rejected",
            "error": record.pop("_registration_error", _BACKLOG_FULL_ERROR),
        }
