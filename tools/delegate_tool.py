...
def _build_child_agent(
    ...
    # agent does. A requested model_profile chain replaces that inherit so
    # a global delegation.model pin cannot retry-to-death on 429.
...
def delegate_task(
    ...
    # used by CLI/gateway startup. When a model_profile is selected, its pool
    # entry supplies the complete route and fallback chain.
    ...
    def _creds_for(profile_name: Optional[str]) -> dict:
        if profile_name not in creds_by_profile:
            creds_by_profile[profile_name] = _credentials_for_model_profile(
                cfg, parent_agent, profile_name
            )
        return creds_by_profile[profile_name]

    try:
        _creds_for(top_profile)
    ...
    live_deleg_id, live_writers, live_paths = create_live_transcripts(
        task_list, context, model=_creds_for(top_profile).get("model"), provider=_creds_for(top_profile).get("provider")
    )
    ...
    if background:
        ...
        dispatch = dispatch_async_delegation_batch(
            ...
            toolsets=None,
            role=top_role,
            model=_creds_for(top_profile)["model"],
...
def _credentials_for_model_profile(
    ...
    """Resolve child credentials from a named model-pool tier."""
    name = str(profile_name or "").strip() or None
    pool = _model_pool(cfg)
    if name is None and pool:
            ...
            "tasks": {
                ...
                "items": {
                    ...
                    "properties": {
                        ...
                        "model_profile": {
                            "type": "string",
                            "description": "Per-task pool tier. Overrides the top-level model_profile.",
                        },
                    },
                    "required": ["goal"],
                },
