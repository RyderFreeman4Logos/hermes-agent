    ...
    def spawn_local(
        ...
        if use_pty:
            ...
            try:
                ...
                pty_argv = _supervised_local_argv(
                ...
                # PtyProcess.spawn can fail after systemd has created the
                # transient scope but before it returns a process handle. In
                # that narrow case the owned scope is the only containment
                # handle; reap it before attempting a distinct pipe fallback.
                if pty_scope_attempted and session._pty is None:
                    contained = _stop_systemd_unit(session.systemd_unit)
                    if not contained:
                        raise RuntimeError(
    ...
    def kill_process(
        ...
        *,
        ...
        if result is not None:
            if result["status"] == "already_exited" and session.systemd_unit:
                # The wrapper can exit while daemonized descendants remain in
                # its transient cgroup. Reap the scope exactly once before
                # honoring either already-exited path.
        ...
        try:
            ...
            # Scope teardown is additive to PID-tree termination and catches
            # descendants reparented inside the worker cgroup.
