...
class ProcessRegistry:
    ...
    def _pty_reader_loop(self, session: ProcessSession):
        ...
        # Process exited: cancel any outstanding canonical-EOF delivery before
        # reaping and closing the PTY handle.
                ...
                if hasattr(session, '_pty') and session._pty:
                    if _IS_WINDOWS:
                        ...
                        try:
                            ...
                            # VEOF flushes pending canonical input as one record.
                            # The owned helper observes the slave queue and sends
                            # the next EOF only after that record is consumed.
                            session._pty.write(eof)
