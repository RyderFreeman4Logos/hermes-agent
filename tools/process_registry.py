...
def _redact_process_result(result: dict) -> dict:
    ...
    verbatim). This is a strict summary boundary: all result fields are forced
    through config and URL-credential redaction regardless of the user setting.
    The command string is also redacted in case it carried an inline credential.
    """
    if not isinstance(result, dict):
        return result
    from agent.redact import redact_sensitive_text
    ...
    if isinstance(result.get("command"), str) and result["command"]:
        result["command"] = redact_sensitive_text(
            result["command"], force=True, code_file=True, redact_url_credentials=True
        )
    return result
