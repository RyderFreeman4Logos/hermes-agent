...
def inspect_process(target_id: str) -> Dict[str, Any]:
    ...
    with session._lock:
        ...
        output_size = len(session.output_buffer)
        pid = session.pid
    ...
    if (
        ...
        try:
            ...
            for process in processes:
                ...
                expected_start = process_registry._safe_host_start_time(process_pid)
                if expected_start is None or (
                    process_pid == pid and expected_start != host_start_time
                ):
                    continue
                try:
                    ...
                    continue
                if process_registry._safe_host_start_time(process_pid) != expected_start:
                    continue
