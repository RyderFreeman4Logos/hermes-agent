...
_CARGO_WRITE_SUBCOMMANDS = frozenset({
    "bench", "build", "check", "clean", "clippy", "doc", "fix", "fmt", "install",
...
def _delegate_command_policy_violation(task_id: Optional[str], command: str) -> Optional[Dict[str, str]]:
    """Return an explicit delegated-command rejection, if the task opted into one."""
    ...
    if policy.get("read_only") is True and any(
        ...
        return {
            "error": "Blocked by delegation.command_policy: write-capable Cargo is prohibited for a read-only delegate.",
            "rule": "read_only",
            "policy_source": source,
        }
    return None
