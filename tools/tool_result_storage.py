...
def maybe_persist_tool_result(content: str, tool_name: str, tool_use_id: str, env=None,
    ...
    appended after persist-or-keep. Non-text results stay inline."""
    ...
    history_limited = len(content) <= configured_threshold and len(content) > effective_threshold
    ...
    if history_limited:
        logger.warning("Keeping recoverable %s result inline after sandbox write failed", tool_name)
        return content + history_suffix
