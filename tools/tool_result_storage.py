...
def maybe_persist_tool_result(
    ...
    Args:
        ...
        config: BudgetConfig controlling thresholds, preview size.
    ...
    # High-volume insertion payloads must be recoverable through the active
    # environment before they are admitted to history. Keep this path aligned
    # with the historical sandbox contract; the host-side spillover path below
    # remains the fallback for other oversized results.
    if (
        threshold is None
        and env is not None
        and _needs_insertion_compaction(tool_name, content)
