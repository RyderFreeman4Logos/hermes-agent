...
def _tui_cache_warm_request(agent) -> dict | None:
    """Build a bodyless copy of the resolved ordinary HTTP Chat route."""
    try:
        from agent.chat_completion_helpers import build_chat_cache_warm_kwargs
        return build_chat_cache_warm_kwargs(agent)
    except Exception:
        logger.debug("bodyless TUI cache warm request construction failed", exc_info=True)
        return None
...
def _tui_cache_warm_identity(agent) -> tuple:
    """Stable local identity of the already resolved physical Chat route."""
    try:
        ...
        cache_scope = None
    return (
        id(agent), getattr(agent, "_openai_transport_generation", 0),
        getattr(agent, "_openai_transport_kind", None),
        str(getattr(agent, "api_mode", "") or ""),
        str(getattr(agent, "provider", "") or ""),
        str(getattr(agent, "model", "") or ""),
        str(getattr(agent, "base_url", "") or ""),
        str(getattr(agent, "session_id", "") or ""), cache_scope,
    )
...
def _cancel_tui_cache_warm(session: dict, *, retain_arm: bool = False) -> None:
    ...
    session.pop("_cache_warm_identity", None)
...
def _tui_cache_warm_due(sid: str, session: dict, agent, route: str, identity=None) -> None:
    identity = identity if identity is not None else session.get("_cache_warm_identity")
    if (
        ...
        or session.get("_cache_warm_identity") != identity
        or _tui_cache_warm_identity(agent) != identity
    ):
        return
    session_key = str(session.get("session_key") or "")
    if not session_key:
    ...
    from hermes_cli.heartbeat import HeartbeatManager
    with _session_profile_runtime_scope(session):
        request = _tui_cache_warm_request(agent)
        token = _notif_claim_turn(session, owner="cache_warm") if request is not None else None
        if token is None:
            return
...
def _arm_tui_cache_warm(sid: str, session: dict, agent, record: dict) -> None:
    if record.get("state") != "hit" or int(record.get("pct") or 0) < _CACHE_WARM_MIN_HIT_PCT:
        return
    if _tui_cache_warm_request(agent) is None:
        return
    route = _tui_cache_warm_route(agent)
    identity = _tui_cache_warm_identity(agent)
    ...
    session["_cache_warm_identity"] = identity
    timer = threading.Timer(
        interval, _tui_cache_warm_due, args=(sid, session, agent, route, identity))
    timer.daemon = True
...
def _tui_cache_warm_miss_classification(session: dict, agent, record: dict) -> str | None:
    if record.get("state") not in {"miss", "cold_write"}:
        return None
    route = _tui_cache_warm_route(agent)
