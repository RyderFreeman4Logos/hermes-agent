...
def _enqueue_prompt(session: dict, text: Any, transport: Any, image_paths: list[str] | None = None,
    ...
    if not structured_completion and text_only and text.strip() == _ac_inflight_original(session) != "":
        return
    queued = {
        "text": text,
        "transport": transport,
    ...
    if (existing and text_only and isinstance(existing.get("text"), str)
            and not existing.get("image_paths") and not existing.get("structured_completion")
            and not structured_completion and not session.get("queued_prompts")):
        prev = existing["text"]
...
def _sanitize_queued_entry_vs_inflight_user(entry: Any, original: str) -> dict | None:
    ...
    if not isinstance(entry, dict):
        return None
    text = entry.get("text")
    if (not original or entry.get("image_paths") or entry.get("structured_completion")
...
def _drain_queued_prompt(rid, sid: str, session: dict) -> bool:
    ...
    try:
        if not use_compute_host:
            accepted = _run_prompt_submit(rid, sid, session, queued["text"], **kwargs)
            if not accepted and completion_events:
                with session["history_lock"]:
